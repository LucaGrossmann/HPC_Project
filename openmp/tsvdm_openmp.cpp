// File:        tsvdm_openmp.cpp
// Description: OpenMP shared-memory parallel implementation of t-SVDM
//              (Kilmer, Horesh, Avron, Newman, 2021, Algorithm 2).
//              Diff from serial/tsvdm_serial.cpp: per-thread dgesdd workspace
//              + #pragma omp parallel for over independent slice loops.
...
// Threading model
// ---------------
// OpenMP parallelizes the outer slice loops only:
//   - sliceSvdAll      : n independent dgesdd calls (the dominant cost)
//   - assembleSDense   : n independent slice fills
//   - V transpose      : n independent slice transposes (inside tsvdm)
//   - reconstructApprox: n independent facewise matmuls (verification only)
// BLAS is called with OPENBLAS_NUM_THREADS=1 to avoid nested parallelism;
// the Slurm script sets this. The mode-3 GEMM stays serial (single dgemm).
 ...
 *
 *  Single dense (mp x n) by (n x n) matmul. Stays serial with BLAS pinned
 *  to one thread; slice SVDs dominate runtime.
 */
...
/** @brief Per-slice thin SVD via LAPACK dgesdd, parallelized over slices.
 *
 *  Each thread holds its own work[] and iwork[] buffers — dgesdd writes
 *  scratch to them, so they cannot be shared across threads.
 */
static void sliceSvdAll(double* Ahat,
    ...
    // Workspace query (single-threaded, before the parallel region).
    int lwork = -1;
    ...
    // Single shared error flag; writes are rare (only on dgesdd failure).
    int firstErr = 0;
    int firstErrSlice = -1;

    #pragma omp parallel
    {
        std::vector<double> work(lwork);     // per-thread scratch
        std::vector<int>    iwork(8 * r);    // per-thread scratch
        int infoLocal = 0;
...
int main(int argc, char** argv) {
    ...
    // Warm up once (pays for any lazy BLAS / OpenMP init) — not counted.
    ...
    std::printf("m=%d p=%d n=%d threads=%d reps=%d rel_err=%.3e "
                "median_ms=%.3f min_ms=%.3f max_ms=%.3f\n",
                m, p, n, nthreads, reps, relErr, medianMs, minMs, maxMs);
